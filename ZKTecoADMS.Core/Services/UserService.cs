...
using ZKTecoADMS.Domain.Constants;
...
public class UserService : IUserService
{
        ...
        if (user != null)
        {
            ...
            foreach (var mapping in mappings)
            {
                var command = new DeviceCommand
                {
                    ...
                    CommandData = $"USERINFO PIN={user.PIN}",
                    Priority = 5
                };
    ...
    public async Task SyncUserToDeviceAsync(Guid userId, Guid deviceId)
    {
        ...
        var userCommand = new DeviceCommand
        {
            DeviceId = deviceId,
            CommandType = "DATA",
            CommandData = $"UPDATE USERINFO PIN={user.PIN}\tName={user.FullName}\tPri={user.Privilege}\tPasswd={user.Password}\tCard={user.CardNumber}\tGrp={user.GroupId}",
            Priority = 3
        };
        ...
        foreach (var fp in user.FingerprintTemplates)
        {
            var fpCommand = new DeviceCommand
            {
                DeviceId = deviceId,
                CommandType = "DATA",
                CommandData = $"UPDATE FP PIN={user.PIN}\tFID={fp.FingerIndex}\tSize={fp.TemplateSize}\tValid=1\tTMP={fp.Template}",
                Priority = 2
            };
        ...
        foreach (var face in user.FaceTemplates)
        {
            var faceCommand = new DeviceCommand
            {
                DeviceId = deviceId,
                CommandType = "DATA",
                CommandData = $"UPDATE FACE PIN={user.PIN}\tFID={face.FaceIndex}\tSize={face.TemplateSize}\tValid=1\tTMP={face.Template}",
                Priority = 2
            };
