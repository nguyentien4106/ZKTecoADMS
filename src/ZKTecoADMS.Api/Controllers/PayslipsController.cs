...
public class PayslipsController(IMediator mediator) : AuthenticatedControllerBase
{
    ...
    public async Task<ActionResult<AppResponse<List<PayslipDto>>>> GetEmployeePayslips(Guid employeeUserId)
    {
        ...
        var query = new GetEmployeePayslipsQuery(employeeUserId, isManagerOrAdmin);
    ...
    {
        var query = new GetEmployeePayslipsQuery(CurrentUserId, IsManager);
        var result = await mediator.Send(query);
        return Ok(result);
    }
    ...
    {
        var query = new GetPayslipByIdQuery(id);
        var result = await mediator.Send(query);
        ...
        // Check if user is viewing their own payslip or is a manager
        var isManagerOrAdmin = IsManager || IsAdmin;
        var currentUserId = CurrentUserId;
